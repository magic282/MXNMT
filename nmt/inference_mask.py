...
class BiS2SInferenceModel_mask(object):
    def __init__(self,
        ...
        self.decode_sym = lstm_attention_decode_symbol(t_num_lstm_layer, t_seq_len, t_vocab_size, t_num_hidden,
                                                       t_num_embed,
                                                       t_num_label, t_dropout, attention, s_seq_len, batch_size)
        ...
        attend_mask = [("encoded_mask", (batch_size, s_seq_len))]
        ...
        decode_input_shapes = dict(target_init_states + decode_data_shape + attend_state_shapes + attend_mask)
    ...
    def decode_forward(self, last_encoded, all_encoded, mask, input_data, new_seq):
        if new_seq:
    ...
    def decode_forward_with_state(self, last_encoded, all_encoded, mask, input_data, state, new_seq):
        if new_seq:
            last_encoded.copyto(self.init_state_executor.arg_dict["encoded"])
            self.init_state_executor.forward()
            init_hs = self.init_state_executor.outputs[0]
...
def lstm_attention_decode_symbol(t_num_lstm_layer, t_seq_len, t_vocab_size, t_num_hidden, t_num_embed, t_num_label,
                                 t_dropout,
                                 attention, source_seq_len, batch_size):
    data = mx.sym.Variable("target")
    encoded_mask = mx.sym.Variable("encoded_mask")
    ...
    all_attended = mx.sym.Reshape(data=all_encoded, shape=(batch_size, source_seq_len, -1),
                                  name='_reshape_concat_attended')
    encoded = mx.sym.SliceChannel(data=all_encoded, axis=1, num_outputs=source_seq_len)
    weights, weighted_encoded = attention.attend(attended=encoded, concat_attended=all_attended,
